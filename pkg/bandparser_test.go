...
func TestParseComboMIMO(t *testing.T) {
	...
	assert.Equal(t, []Band{b1, b1}, entries[0].Bands())
	assert.Equal(t, []Band{b1_2}, entries[1].Bands())
	assert.IsType(t, &DownlinkEntry{}, entries[0])
	assert.IsType(t, &UplinkEntry{}, entries[1])
}
